...
grep -o '<td class="date">[[:space:]]*\([^<]*\)</td>[[:space:]]*<td class="name">[[:space:]]*\([^<]*\)[[:space:]]*</td>[[:space:]]*<td>\([^<]*\)</td>' < tmp/nospaces.html | sed -e 's/<td class="date">[[:space:]]*\([^<]*\)<\/td>[[:space:]]*<td class="name">[[:space:]]*\([^<]*\)[[:space:]]*<\/td>[[:space:]]*<td>\([^<]*\)<\/td>/\1\n\2\n\3/g' > marres
